...
达梦驱动（`database/dm.go`）使用达梦官方 Go 驱动（`third_party/dm/dm/`），通过 `replace` 指令引入：
...
**达梦驱动** — 使用达梦官方 Go 驱动（`third_party/dm/dm/`），通过 `replace` 指令引入。驱动已实现完整的 SQL 查询、事务、表结构查询功能。
...
Go 1.26, mark3labs/mcp-go, go-sql-driver/mysql, jackc/pgx/v5, modernc.org/sqlite, github.com/redis/go-redis/v9, 达梦官方 Go 驱动（third_party/dm/）, fsnotify, yaml.v3
