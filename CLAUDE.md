...
**dbmcp** v0.5.0 — 一个用 Go 编写的数据库操作 MCP Server。通过模型上下文协议（stdio 传输）暴露 15 个工具，供 AI 工具与 MySQL、PostgreSQL、SQLite、SQL Server、达梦、Redis 数据库交互。支持多数据库连接、结构化配置、per-database 权限、配置自动迁移、事务控制、配置热重载、首次运行自动生成默认配置、SQL 注入防护和操作审计。详见 [数据库支持文档](docs/zh/databases.md)。
...
> **注意**：SQLite 使用 `modernc.org/sqlite`（纯 Go 实现），无需 CGO。**达梦驱动需要 `-tags dm` 构建**。MySQL/PostgreSQL 集成测试需要 Docker 自动启动容器。
...
| `permission/` | 权限校验：per-database 权限（每个数据库独立配置）、只读模式、数据库白名单、表黑名单、操作白名单、Redis 命令白名单/key 黑名单 |
...
旧格式（扁平 `databases` map + `permissions`）仍然有效，加载时会自动迁移到分组结构并备份为 `config.yaml.bak`。迁移包括：数据库按类型分配到 `database_groups`，旧权限展开为 per-database 配置。
...
达梦驱动（`database/dm.go`）使用达梦官方 Go 驱动（`third_party/dm/dm/`），通过 `replace` 指令引入。**构建时需要 `-tags dm` 参数**：

```bash
go build -tags dm -o build/dbmcp.exe ./cmd/dbmcp
```

- **DSN 格式** — `dm://user:password@host:port?autoCommit=0&schema=模式名`
- **事务支持** — 默认 `autoCommit=0`，支持 `begin_tx` → `execute_update` → `commit`/`rollback`
- **模式支持** — 通过 `schema` 参数设置默认模式，表名使用 `"模式名"."表名"` 格式
- **参数化查询** — 使用 `?` 占位符，通过 `execute_param_query` 工具执行
- **工具表** — 在 `manager.go` 中注册 `dm`/`dmdbms`/`dameng` 三种类型

### 添加新数据库驱动

实现 `internal/database/interface.go` 中的 `DatabaseDriver` 接口（含事务方法），然后在 `manager.go` 的 `createDriver` 工厂函数中注册。

**达梦驱动** — 使用达梦官方 Go 驱动（`third_party/dm/dm/`），通过 `replace` 指令引入。需 `-tags dm` 构建。驱动已实现完整的 SQL 查询、事务、表结构查询功能。
...
Go 1.26, mark3labs/mcp-go, go-sql-driver/mysql, jackc/pgx/v5, modernc.org/sqlite, github.com/redis/go-redis/v9, microsoft/go-mssqldb, 达梦官方 Go 驱动（third_party/dm/，需 `-tags dm` 构建）, fsnotify, yaml.v3
